...
drug_sim = torch.from_numpy(np.loadtxt('./data/drugfusimilarity.txt')) #1373
...
def load_data():
    ...
    drug_simi_fu = torch.from_numpy(np.loadtxt("./data/drugfusimilarity.txt"))
    drug_simi_he=torch.from_numpy(np.loadtxt("./data/drughesimilarity.txt" ))
...
torch.save([fea1,fea2, train_xy, test_xy,asso_mat,asso_mat_mask],'./embed.pth')
